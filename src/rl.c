...
int main() {
  ...
  Vector2 playerPosition = {(float)screenWidth / 2.0f,
  ...
  while (!WindowShouldClose()) {
    ...
    if (((IsKeyDown(KEY_UP) || IsKeyDown(KEY_W)) &&
        ...
        ((IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_A)) &&
         (IsKeyDown(KEY_RIGHT) ||
          IsKeyDown(KEY_D)))) { // speed is zero and thus is not moving
      currentSpeed = 0;
      playerMoving = false;
    } else if (IsKeyDown(KEY_W) || IsKeyDown(KEY_UP) || IsKeyDown(KEY_A) ||
               IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_S) || IsKeyDown(KEY_DOWN) ||
    ...
    if (false) {
      printf("%.2f\t%.2f\t%.2f\t", playerPosition.x, circleRadius,
             cos(DEG2RAD * (float)playerDirection));
      printf("%.2f\t%.2f\t%.2f\n", playerPosition.y, circleRadius,
             sin(DEG2RAD * (float)playerDirection));
    }

    lineStart.x = playerPosition.x;
    lineStart.y = playerPosition.y;
    ...
    // if (currentFPS <= 0) fpsText = TextFormat("FPS: unlimited (%i)",
    // GetFPS()); else fpsText = TextFormat("FPS: %i", GetFPS());
    ...
    DrawText(TextFormat("Player moving? %c | speed? %f", playerMovingText,
                        (playerMoving) ? currentSpeed : 0),
             10, 10, 20, YELLOW);

    // player position
    DrawText(TextFormat("Player Position: \nX: %f\nY: %f", playerPosition.x,
                        playerPosition.y),
             10, 70, 20, GREEN);

    // player direction
    DrawText(TextFormat("Player Direction: %s\t(%d deg)", directionText,
                        playerDirection),
             10, 40, 20, BLUE);
...
Vector2 calcLineEnd(Vector2 pPos, float r, Direction d) {
  ...
  case RIGHT:
    ...
    break;
  case UPRIGHT:
    total.x = pPos.x + (r * cos(d));
    total.y = pPos.y + (r * sin(d));
    break;
  case UPLEFT:
    total.x = pPos.x + (r * cos(d));
    total.y = pPos.y + (r * sin(d));
